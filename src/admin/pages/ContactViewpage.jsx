...
function ContactViewpage() {
  ...
  Contact
            ...
            <p className='text-slate-500 font-medium leading-tight m-2 dark:text-white'>Manage and respond to user inquiry</p>
            </div>
            ...
            <Tabs aria-label="Default tabs" variant="default">
              <TabItem active title="All-Inquiry" >
      ...
      <Table>
        ...
        <TableBody className="divide-y">
          { contactData.map((data,index)=>( <TableRow className="bg-white dark:border-gray-700 dark:bg-gray-800" key={index}>
            <TableCell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">
              {data.username}
            </TableCell>
            {/* <TableCell>{data.email}</TableCell> */}
            <TableCell>{data.inquiryType}</TableCell>
            <TableCell>{data.message}</TableCell>
            <TableCell><a href={`tel:${data.phone}`} className="hover:text-blue-800 hover:underline">{data.phone}</a></TableCell>
            <TableCell>
              <a href={`mailto:${data.email}?subject=inquiry for ${data.inquiryType} Request&body=Hello Sir/mam`} className="font-medium text-primary-600 hover:underline dark:text-primary-500">
                replay
              </a>
            </TableCell>
          </TableRow>))}
